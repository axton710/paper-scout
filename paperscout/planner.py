...
5. 为每个子方向明确 `scope`（本 agent 必须覆盖的具体问题）和 `exclude`（已由其他方向承担、不应作为本方向重点的内容）。边界用于后续识别检索重叠，不要求彼此绝对互斥。
6. 只做上面的事，**不要写综述正文**。
...
{{
  ...
  "subtopics": [
    {{"name": "子方向名", "rationale": "为什么要覆盖它", "scope": "本方向必须回答的问题", "exclude": "不作为本方向重点的范围", "queries": ["english query 1", "english query 2"]}}
  ]
}}
```
